...
/**
 * Created by dragos on 5/25/15.
 */
public class AreaCalculator {
    private static AreaCalculator ourInstance = new AreaCalculator();
    ...
    public double calculateAreaOfGPSPolygonOnEarthInSquareMeters(final List<com.agro.gusutri.agroconsult.model.Location> locations) {

        List<Location> aLocations= new ArrayList<>();
        for(com.agro.gusutri.agroconsult.model.Location l: locations){
            Location newLoc= new Location("");
            newLoc.setLatitude(l.getLatitude());
            newLoc.setLongitude(l.getLongitude());
            aLocations.add(newLoc);
        }
        return calculateAreaOfGPSPolygonOnSphereInSquareMeters(aLocations, EARTH_RADIUS);
    }
    ...
    private double calculateAreaOfGPSPolygonOnSphereInSquareMeters(final List<Location> locations, final double radius) {
        ...
        for (int i = 1; i < locations.size(); i++) {
            ...
            Log.d("calc Area", String.format("Y %s: %s", listY.size() - 1, listY.get(listY.size() - 1)));
            listX.add(calculateXSegment(longitudeRef, longitude, latitude, circumference));
            Log.d("calc Area", String.format("X %s: %s", listX.size() - 1, listX.get(listX.size() - 1)));
        }
        ...
        for (int i = 1; i < listX.size(); i++) {
            ...
            Log.d("calc Area", String.format("area %s: %s", listArea.size() - 1, listArea.get(listArea.size() - 1)));
        }
